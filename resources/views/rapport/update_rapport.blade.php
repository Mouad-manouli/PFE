...
<body>
    <section class="mx-auto">
        ...
        @if (!session('materiels'))
            <form action="{{route('update.rapport1',$Rapport->id)}}" method="POST">
                ...
                @method('put')
                    ...
                    <p> Please fill out this form to edit your report. </p>
            </form>
        @endif
    
        @if(session('materiels')  && session('id_m'))
            <form action="{{route('update.rapport2',$Rapport->id)}}" method="POST">
                <a href="/profile/employee">Back 
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-right-short" viewBox="0 0 16 16">
                        <path fill-rule="evenodd" d="M4 8a.5.5 0 0 1 .5-.5h5.793L8.146 5.354a.5.5 0 1 1 .708-.708l3 3a.5.5 0 0 1 0 .708l-3 3a.5.5 0 0 1-.708-.708L10.293 8.5H4.5A.5.5 0 0 1 4 8"/>
                    </svg>
                ...
                @method('put')
                    ...
                    <input type="date" name="DateDebut" value="{{ old('DateDebut',$Rapport->DateDebut) }}" class="form-control">
                    <div id="emailHelp" class="form-text text-danger">
                        @error('DateDebut')
                            {{$message}}
                        @enderror 
                    ...
                    <input type="date" name="DateFin" value="{{ old('DateFin',$Rapport->DateFin) }}" class="form-control"> 
                    <div id="emailHelp" class="form-text text-danger">
                        @error('DateFin')
                            {{$message}}
                        @enderror 
                    ...
                    <input type="text" name="Etat_machine" value="{{ old('Etat_machine',$Rapport->Etat_machine) }}" class="form-control">
                    <div id="emailHelp" class="form-text text-danger">
                        @error('Etat_machine')
                            {{$message}}
                        @enderror 
                    ...
                    <textarea name="Description" cols="30" rows="0" class="form-control">{{ old('Description',$Rapport->Description) }}</textarea>
                    <div id="emailHelp" class="form-text text-danger">
                        @error('Description')
                            {{$message}}
                        @enderror 
